...
router.post('/cargarHoras', isLoggedIn, async (req, res) => {
   ...
   var start = new Date(dateFormat(fechaI, "dd-mm-yyyy"));
   var end = new Date(dateFormat(fechaT, "dd-mm-yyyy"));
   ...
   while(fechaX <= end){
       ...
       if (fechaX.getDay() == 5) // es viernes 
       {
            ...
            if (analisisHoraI < analisisHoraAlmuerzoViernes)
            {
                ...
                {
                    var difference = dateFormat(dateFormat(fechaX,"dd-mm-yyyy") + " " + "13:30:00", "yyyy-mm-dd h:MM:ss") - dateFormat(dateFormat(fechaX,"dd-mm-yyyy") + " " + horaI + ":00", "yyyy-mm-dd h:MM:ss"); 

                    var startTime = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + hi + ":00"); 
                    var endTime = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " +"13:30:00");
                ...
                else
                {
                    ...
                    var startTime = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + hi + ":00"); 
                    var endTime = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + hT + ":00");
                    var difference = endTime.getTime() - startTime.getTime(); // This will give difference in milliseconds
                    var resultInHours = Math.round(difference / 60000) / 60;
       ...
       else
       {
        ...
        if(analisisHoraI < analisisHoraAlmuerzoI && analisisHoraFI < analisisHoraAlmuerzoI) // revisar si las dos fechas son menores de la hora de almuerzo 
        {
            ...
            //console.log(resultInMinutes);
            //console.log(dateFormat(dateFormat(fechaX,"yyyy-mm-dd") + " " + hi + ":00", "yyyy-mm-dd h:MM:ss"));
            const newRegistroBitacora  ={ //Se gurdaran en un nuevo objeto
                                            ini_time: dateFormat(dateFormat(fechaX,"yyyy-mm-dd") + " " + hi + ":00", "yyyy-mm-dd HH:MM:ss") ,
                                            fin_time: dateFormat(dateFormat(fechaX,"yyyy-mm-dd") + " " + hT + ":00", "yyyy-mm-dd HH:MM:ss") ,
                                            title:proyeto[0].year + "-" +proyeto[0].code + " : " + proyeto[0].nombre,
        ...
        else if ( (analisisHoraI < analisisHoraAlmuerzoI || analisisHoraI < analisisHoraAlmuerzoF) && (analisisHoraFI >= analisisHoraAlmuerzoF || analisisHoraFI >= analisisHoraAlmuerzoI))
        {
            ...
            var startTimeAM = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + hi + ":00"); 
            var endTimeAM = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + "13:00:00");
            ...
            var startTimePM = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " +"14:00:00"); 
            var endTimePM = new Date(dateFormat(fechaX,"dd-mm-yyyy") + " " + hT +":00");
